...
declare const $: (selector: string) => {
  DataTable: (opts: unknown) => { clear: () => unknown; draw: () => void };
};
declare const Popper: {
  createPopper: (ref: Element, tooltip: HTMLElement) => { destroy: () => void };
};
...
const setupTableData = async (
  ...
  tableData = tableData.map((row) => {
    const r = row as Record<string, unknown> & {
      link_html?: HTMLElement;
      personaname_html?: HTMLElement;
      vac_html?: HTMLElement;
      profile?: string;
      os?: string;
    };
    const gameNickWords =
      (r.link_html as HTMLElement)?.innerText?.replace(/[\W_]+/g, '').trim() ?? '';
    const steamNickWords =
      (r.personaname_html as HTMLElement)?.innerText?.replace(/[\W_]+/g, '').trim() ?? '';
  ...
  table = $('#dataTable').DataTable({
    ...
    columns: [
      ...
      },
      {
        ...
        data: 'summary',
        render: (data: unknown) =>
          ((data as string) === 'No information given.' ? '' : data) as string,
        defaultContent: '',
      },
    ],
    columnDefs: [
