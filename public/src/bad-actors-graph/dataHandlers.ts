...
export const onBansData = (players: unknown[]): void => {
  (players as { SteamId: string; CommunityBanned?: boolean; VACBanned?: boolean }[]).forEach(
    (player) => {
      const playerEntry = STATE.graphLookup[player.SteamId] as Record<string, unknown>;
      if (playerEntry) {
...
export const onXMLData = (xml: string, id: string): void => {
  ...
  if (playerEntry) {
    ...
    playerEntry.summary = xmlDoc.getElementsByTagName('summary')[0]?.childNodes[0]?.nodeValue ?? '';
  }
};
